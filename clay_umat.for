...
c
        ...
        alpha_power=parms(21)
c
        sinphi=dsin(phi)
        sinphi2=sinphi*sinphi
        nuvh=nuhh/alphanu
        if(alpha_power.lt. 0.01) then
...
!           write(1,*) 'vertical direction can only be 1/2/3'
	...
	LL(6,6)=LLfour(2,3,2,3)
        ...
        kpow=1.7+3.9*sin(phi)*sin(phi)
	sinphickpow=(sin(phi))**kpow
 	sinphimkpow=(sqrt(sin2phim))**kpow
	
	Amult=2./3-sqrt(sqrt(sin2phim))*(cos3t+1)/4.
...
c
      ...
      double precision lambda,kappa,Am,pmeangt1,p_t
      integer istrain
