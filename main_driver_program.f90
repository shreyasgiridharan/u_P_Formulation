    ...
    double precision, dimension(:), allocatable :: GrvF, InrF, Mas, Area, Areai, ExtF, v0, v, dis, DamF, InrFFl, MasFl, A, GrvFFl,GrvForFl, FVelFl, PresFl, a0, VolFl
    ...
    double precision, dimension(:,:,:), allocatable :: SigG, EpsG, F, HS, B_trial, EpsP, EpsE, edP, ETA, Sig0, &
        Statevar, Swp, Dswp, Plastind, FBARNEW, Pressure, SigE
    ...
    allocate(MasFl(2 * NNod)); MasFl = 0.d0
    allocate(FVelFl(2 * NNod)); FVelFl = 0.d0
    allocate(GrvF(2 * NNod)); GrvF = 0.d0
    allocate(GrvFFl(2 * NNod)); GrvFFl = 0.d0
    allocate(GrvForFl(2 * NNod)); GrvForFl = 0.d0
    allocate(InrF(2 * NNod)); InrF = 0.d0
    allocate(InrFFl(2 * NNod)); InrFFl = 0.d0
    allocate(ExtF(2 * NNod)); ExtF = 0.d0
    allocate(PresFl(2 * NNod)); PresFl = 0.d0
    allocate(DamF(2 * NNod)); DamF = 0.d0
    allocate(VolFl(2 * NNod)); VolFl = 0.d0
    allocate(v(2 * NNod)); V = 0.d0
    allocate(A(2 * NNod)); A = 0.d0
    allocate(a0(2 * NNod)); a0 = 0.d0
    ...
    double precision :: factor, satmass = 0.d0, flmass = 0.d0
    GrvF = 0.D0; InrF = 0.D0; ExtF = 0.D0; Mas = 0.D0
    InrFFl = 0.d0; MasFl = 0.d0; GrvFFl = 0.d0; GrvForFl = 0.d0; FVelFl = 0.d0; PresFl = 0.d0; DamF = 0.d0; VolFl = 0.d0
    ...
    do IEl = 1, nel
        ...
        do ig = 1, 4 ! gauss
            do I = 1, 4
                Id = (INod(I) - 1) * 2
                InrF(Id + 1) = InrF(Id + 1) + ((SigE(1,ig,IEl)) * B(1,I,iel,ig) + (SigE(3,ig,IEl)) * B(2, I, iel, ig)) * Area(iel)/4.d0 + ((Pressure(1,ig,IEl)) * B(1,I,iel,ig) + (Pressure(3,ig,IEl)) * B(2, I, iel, ig)) * Area(iel)/4.d0
                InrF(Id + 2) = InrF(Id + 2) + ((SigE(3,ig,IEl)) * B(1,I,iel,ig) + (SigE(2,ig,IEl)) * B(2, I, iel, ig)) * Area(iel)/4.d0 + ((Pressure(3,ig,IEl)) * B(1,I,iel,ig) + (Pressure(2,ig,IEl)) * B(2, I, iel, ig)) * Area(iel)/4.d0
                satmass = 0.d0; satmass = (1.d0 - MatProp(5))*MatProp(3) + MatProp(5)*MatProp(4)
                GrvF(Id + 1) = GrvF(Id + 1) + Area(iel) * satmass * hs(i, iel, ig) * gx /4.d0
                GrvF(Id + 2) = GrvF(Id + 2) + Area(iel) * satmass * hs(i, iel, ig) * gy /4.d0
                Mas(Id + 1) = Mas(Id + 1)   + Area(iel) * MatProp(3) * hs(i, iel, ig) /4.d0
                Mas(Id + 2) = Mas(Id + 2)   + Area(iel) * MatProp(3) * hs(i, iel, ig) /4.d0

                InrFFl(Id + 1) = ((Pressure(1,ig,IEl)) * B(1,I,iel,ig) + (Pressure(3,ig,IEl)) * B(2, I, iel, ig)) * Area(Iel) / 4.d0
                InrFFl(Id + 2) = ((Pressure(3,ig,IEl)) * B(1,I,iel,ig) + (Pressure(2,ig,IEl)) * B(2, I, iel, ig)) * Area(Iel) / 4.d0
                VolFl(Id + 1) = VolFl(Id + 1) + hs(i, iel, ig) * Area(Iel) / 4.d0
                VolFl(Id + 2) = VolFl(Id + 2) + hs(i, iel, ig) * Area(Iel) / 4.d0
                flmass = 0.d0; flmass = MatProp(4) * MatProp(5)
                !flmass = 0.d0; flmass = MatProp(4) 
                MasFl(Id + 1) = MasFl(Id + 1) + flmass *  hs(i, iel, ig) * Area(Iel) / 4.d0
                MasFl(Id + 2) = MasFl(Id + 2) + flmass *  hs(i, iel, ig) * Area(Iel) / 4.d0 
                GrvFFl(Id + 1) = GrvFFl(Id + 1) + flmass *  hs(i, iel, ig) * gx  * Area(Iel) / 4.d0
                GrvFFl(Id + 2) = GrvFFl(Id + 2) + flmass *  hs(i, iel, ig) * gy  * Area(Iel) / 4.d0
                GrvForFl(Id + 1) = GrvForFl(Id + 1) + flmass *  hs(i, iel, ig) * 10.d0  * Area(IEl) / 4.d0
                GrvForFl(Id + 2) = GrvForFl(Id + 2) + flmass *  hs(i, iel, ig) * 10.d0  * Area(IEl) / 4.d0
                FVelFl(Id + 1) = FVelFl(Id + 1) + flmass *  hs(i, iel, ig) * A(Id + 1)  * Area(IEl) / 4.d0
                FVelFl(Id + 2) = FVelFl(Id + 2) + flmass *  hs(i, iel, ig) * A(Id + 2)  * Area(IEl) / 4.d0
            end do
        enddo !gauss
    end do !nel
    ...
    do IEl = 1, nel
        ...
        do ig = 1, 4 ! gauss
            do I = 1, 4
                Id = (INod(I) - 1) * 2
                PresFl(Id + 1) = PresFl(Id + 1) + (-InrFFl(Id + 1) + FVelFl(Id + 1) - GrvFFl(Id + 1))!/VolFl(Id + 1)
                PresFl(Id + 2) = PresFl(Id + 2) + (-InrFFl(Id + 2) + FVelFl(Id + 2) - GrvFFl(Id + 2))!/VolFl(Id + 2)
            end do
        enddo !gauss
    end do !nel
    ...
    double precision :: delV(4), deps(4), tem, dampf = 0.01d0, check, epsilon, rad, delW(4), dPore, factor
    ...
    do IEl = 1, NEl
        ...
        do ig = 1, 4 ! gauss
            ...
            do I = 1, 4
                ...
                delW(1) = delw(1) + PresFl(Id + 1) * B(1, i, iel, ig)
                delW(2) = delW(2) + PresFl(Id + 2) * B(2, i, iel, ig)
                delW(3) = delW(3) + GrvForFl(Id + 1)
                delW(4) = delW(4) + GrvForFl(Id + 2)
            end do
            ...
            dPore = 0.d0; 
            !dPore =  - dt * MatProp(7)/MatProp(5) * ((MatProp(6)/delW(3)*dwelW(1) + delV(1)) + (MatProp(6)/delW(4)*delW(2) + delV(2)))
            dPore =  dt * MatProp(7)/MatProp(5) * ((MatProp(6)/delW(3)*delW(1) + delV(1)) + (MatProp(6)/delW(4)*delW(2) + delV(2))) * dt
            ...
            write(Logunit,*) itime*dt, Pressure(4,ig,1), dPore

            !if(itime*dt.lt.4.d0) then
            !    factor = itime*dt/4.d0
            !else
            !   factor = 1.d0
            !end if
            !
            !
            !if(itime*dt.lt.4.d0) then
            !Pressure(1,ig,iel) = -5.d0 * factor
            !Pressure(2,ig,iel) = -5.d0 * factor
            !Pressure(3,ig,iel) = -0.d0 * factor
            !Pressure(4,ig,iel) = -5.d0 * factor
            !end if
...
111 format(I7, 2E16.6E3)
    ...
    write(Unit, *) 'Result "velocity" "MPM"', IStep, 'Vector OnNodes'
    write(Unit, *) 'ComponentNames "vel. x", "vel. y"'
    write(Unit, *) 'values'
    do INod = 1, NNod
        Id = (INod - 1) * 2
        write(Unit, "(I7, 2E16.6E3)") INod, V(Id + 1), V(Id + 2)
    end do
    write(Unit, *) 'end values'
